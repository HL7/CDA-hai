...
<!-- $LastChangedDate: 2013-06-18 03:47:03 +1000 (Tue, 18 Jun 2013) $ -->
<!-- ============================================================== -->
...
<xsl:transform version="1.1" xmlns="urn:hl7-org:v3" xmlns:cda="urn:hl7-org:v3"
  ...
  <!-- ============================================================== -->
  <!-- Vocabulary handling -->
  <!-- read voc.xml into a lookup table -->
  ...
  <xsl:template match="*[not(self::ClinicalDocument) and not(self::cda:section)][cda:templateId]"
    ...
    <xsl:choose>
      ...
      <xsl:when test="cda:templateId/@root='2.16.840.1.113883.10.20.5.6.166'">
        <xsl:call-template name="pattern-2"/>
      </xsl:when>
 ...
 ">
      ...
      <xsl:when
        test="cda:templateId/@root='2.16.840.1.113883.10.20.5.2.2.7.2'
                   ...
                   or cda:templateId/@root='2.16.840.1.113883.10.20.5.6.153'
...
-->
  ...
  <xsl:template match="cda:encounter[cda:templateId/@root='2.16.840.1.113883.10.20.5.6.151']"
    mode="exceptions">
    <tr>
      <td valign="top">Prior Encounter</td>
      <td valign="top">
        <xsl:for-each select="cda:code">
          <xsl:call-template name="get-displayName"/>
        </xsl:for-each>
  ...
  <!-- Pattern exception: Specimen Collection Procedure (LIO) 2.16.840.1.113883.10.20.5.6.165 
    and Specimen Collection Procedure (ARO) 2.16.840.1.113883.10.20.5.6.188
 -->
  <xsl:template
    match="cda:procedure[cda:templateId/@root='2.16.840.1.113883.10.20.5.6.165'
    or cda:templateId/@root='2.16.840.1.113883.10.20.5.6.188']"
    mode="exceptions">
    <tr>
      <td>Specimen type</td>
  ...
  <xsl:template
    ...
    <tr>
      <td valign="top">
        <!-- change context node -->
        <xsl:for-each
          select="cda:consumable/cda:manufacturedProduct/cda:manufacturedLabeledDrug/cda:code">
          <xsl:call-template name="get-displayName"/>
        </xsl:for-each>
      </td>
      <td valign="top">
  ...
  <!-- Pattern Exceptions:
              Procedure Details in a Procedure Report     ...5.6.153
  ...
  <xsl:template
    match="cda:observation[cda:templateId/@root='2.16.840.1.113883.10.20.5.6.115']"
    mode="exceptions">
    <br/>
    <!-- change context node -->
    <xsl:for-each select="cda:value">
