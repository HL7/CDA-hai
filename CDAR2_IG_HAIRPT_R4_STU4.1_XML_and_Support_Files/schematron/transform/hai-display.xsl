...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3"
  ...
  <!-- Last reviewed for ballot updates              KH / Nov 24 2009
       ...
       Update for HV to display tables better        SG / Jun 15 2016-->
...
-->
  ...
  <xsl:template name="participant-encounter-location">
    <table class="header_table">
      <tbody>
        <xsl:for-each
          select="n1:participant[@typeCode='LOC'][@contextControlCode='OP']
                                /n1:associatedEntity[@classCode='SDLOC'][n1:id]">
          <tr>
            <td width="20%" bgcolor="#3399ff">
              <span class="td_label">
  ...
  <xsl:template match="n1:td">
    <td>
      <xsl:copy-of select="@*" />
      <!-- SG: Added last condition so that if a column is set in the XML then that will override the 35% setting -->
      <xsl:if test="ancestor::n1:text and not(preceding-sibling::n1:td) and not(ancestor::n1:table/child::n1:col)">
